...
///////////////////////////////////////
extern std::vector<RegionInfoManager*> scan_b_;
///////////////////////////////////////
...
static int64_t TotalFileSize(const std::vector<FileMetaData*>& files) {
  ...
  //   for(int j = 0; j < files[i]->patch_file.size(); j++){
  //     sum += files[i]->patch_file[j].file_size;
  //   }
  // }
  for (size_t i = 0; i < files.size(); i++) {
...
///////////////////////////////////////
class Version::LevelClusterNumIterator : public Iterator {
 public:
  LevelClusterNumIterator(const InternalKeyComparator& icmp,
...
static Iterator* GetMixedterator(void* arg,
  ...
  //std::vector<RegionInfo>* chosenPosition = reinterpret_cast<std::vector<RegionInfo>*>(arg2);
  //const InternalKeyComparator icmp_;
  ...
  //      Status::Corruption("FileReader invoked with unexpected value"));
  //} else {
    //std::vector<RegionInfo>* chosenPosition = &(version->current()->valid
  Iterator* it = cache->NewMixedIterator(options,
                            c->GetInputVersion()->GetVset()->Icmp(), 
                            &(c->input(which, index)->patch_file),
                            (c->input(which, index)->rim_->ValidRegion()));
  return it;
  //}
}
///////////////////////////////////////
...
static Iterator* ScanGetMixedterator(void* arg,
  ...
  Iterator* it = cache->NewMixedIterator(options,
                            v->GetVset()->Icmp(), 
                            &((*files)[index]->patch_file),
                            ((*files)[index]->rim_->ValidRegion()));
  (*files)[index]->rim_->refs++;
  scan_b_.push_back((*files)[index]->rim_);
  return it;
  //}
}
///////////////////////////////////////
...
Iterator* Version::FGKVNewConcatenatingIterator(const ReadOptions& options,
                                            int level) const {
  return NewTwoLevelIterator(
      new LevelClusterNumIterator(vset_->icmp_, level, &files_[level]),
  ...
  for (size_t i = 0; i < files_[0].size(); i++) {
    iters->push_back(
        vset_->table_cache_->NewMixedIterator(options, GetVset()->icmp_, 
                &(files_[0][i]->patch_file), files_[0][i]->rim_->ValidRegion()));
  }
 ...
 public:
  ...
  void Apply(VersionEdit* edit) {
    ...
    for (size_t i = 0; i < edit->new_files_.size(); i++) {
      ...
      // 这一步是因为有些通过IsTrivialMove途径下来的sstable只是元数据换成新的，
      // 但是磁盘文件不应该被删除
...
Status VersionSet::WriteSnapshot(log::Writer* log) {
  ...
  for (int level = 0; level < config::kNumLevels; level++) {
    ...
    for (size_t i = 0; i < files.size(); i++) {
      const FileMetaData* f = files[i];
      ///////////////////////////////////////
      edit.AddFile(level, f->number, f->file_size, f->smallest, f->largest, f->rim_, f->patch_file, f->fmd_stage_);
      ///////////////////////////////////////
    }
  }
...
///////////////////////////////////////
// 陈冠中添
#ifndef DISABLE_PATCH_PLUS_COMPACTION
void VersionSet::GetchosenRegions(Compaction* c,
                 std::vector< std::vector<RegionInfo*> >* chosenRegion,
                 Iterator* iiter) {
  //// <FG_STATS>
  uint64_t fg_gcr_start = env_->NowMicros();

  for (size_t i = 0; i < c->inputs_[1].size(); i++) {
    c->fg_delete_size_.push_back(0);
    FileMetaData* f = c->inputs_[1][i];
    RegionInfoManager* rIM = f->rim_;
    std::vector<RegionInfo*> pVec;
    if (c->fg_drop_[i]) {
      chosenRegion->push_back(*(rIM->ValidRegion()));
      ...
      uint64_t fg_region_size = 0;
      int patchNumber = f->patch_file.size();
      uint64_t* deletedSizeEachPatch = new uint64_t[patchNumber]();
      // 需要全选的patch
      std::vector<int> chosenPatch;
      std::vector<int> chosenPosition;
      bool moved = true;
      std::vector<RegionInfo*>* vR = rIM->ValidRegion();
      int pos = 0, vRSize = vR->size();
      while (iiter->Valid()) {
        while (pos < vRSize && 
              ExtractUserKey(iiter->key()).compare(ExtractUserKey((*vR)[pos]->index_key)) > 0) {
          pos++;
          moved = true;
        }
        if (pos < vRSize && moved) {
          // pVec.push_back((*vR)[pos]);
          chosenPosition.push_back(pos);
          f->patch_file[(*vR)[pos]->file].invalid_regions_count += 1;

          Slice fg_block_handle((*vR)[pos]->block_handle);
          if (GetVarint64(&fg_block_handle, &fg_region_size) &&
                    GetVarint64(&fg_block_handle, &fg_region_size)) { 
            //c->fg_delete_size_[i] += fg_region_size;
            deletedSizeEachPatch[(*vR)[pos]->file] += fg_region_size;
            f->patch_file[(*vR)[pos]->file].invalid_size += fg_region_size;
          }
      ...
      for (int patchNumber = 0; patchNumber < f->patch_file.size(); patchNumber ++) {
        if (f->patch_file[patchNumber].is_valid && 
              f->patch_file[patchNumber].invalid_size > f->patch_file[patchNumber].file_size * 0.8) {
          chosenPatch.push_back(patchNumber);
          f->patch_file[patchNumber].invalid_size = f->patch_file[patchNumber].file_size;
          f->patch_file[patchNumber].invalid_regions_count = f->patch_file[patchNumber].total_regions_count;
        }
      }
      int start = 0;
      int cpSize = chosenPatch.size();
      if (!chosenPosition.empty()) {
        for (int j = 0; j < vRSize; j++) {
          // 已经被选中的
          if (j == chosenPosition[start]) {
            pVec.push_back((*vR)[j]);
            start += 1;

            // Slice fg_block_handle((*vR)[j]->block_handle);
            // if (GetVarint64(&fg_block_handle, &fg_region_size) &&
            //           GetVarint64(&fg_block_handle, &fg_region_size)) { 
            //   c->fg_delete_size_[i] += fg_region_size;
            // }
      ...
      for (int k = 0; k < patchNumber; k++) {
        if (f->patch_file[k].is_valid && f->patch_file[k].invalid_size >= f->patch_file[k].file_size) {
          c->fg_delete_size_[i] += f->patch_file[k].file_size;
        } else {
          c->fg_delete_size_[i] += deletedSizeEachPatch[k];
        }
        originalDeletedSize += deletedSizeEachPatch[k];
      }
      // // 如果该簇sstable没有一个命中块，那就选中它的最后一个block
      // // (这是有可能的，比如该簇sstable中只有寥寥数个kv对，inputs_[0]完全有可能
      // // 没有kv对对应它的block，但是inputs_[0]整体的key range又覆盖该簇sstable)
      // if (pVec.size() == 0) {
      //   pVec.push_back((*vR)[--pos]);
      ...
      if (!c->fg_drop_[i]) {
        fg_stats_->chosen_patch += cpSize;
        for (int cc = 0; cc < cpSize; cc++) {
          fg_stats_->chosen_patch_size += c->inputs_[1][i]->patch_file[cc].file_size;
        }
      }
      // // 如果没有命中块，不删除它的元数据
      // if (pVec.empty()) c->fg_drop_[i] = false;
      chosenRegion->push_back(pVec);
    } // <end if(簇合并) ... else>
  } // <end for (size_t i = 0; i < c->inputs_[1].size(); i++)>
  
  //// <FG_STATS>
  fg_stats_->fg_getchosenregions_time += (env_->NowMicros() - fg_gcr_start);
  ...
  for (Version* v = dummy_versions_.next_;
    ...
    for (int level = 0; level < config::kNumLevels; level++) {
      ...
      for (size_t i = 0; i < files.size(); i++) {
        ...
        for(int j = 0; j < files[i]->patch_file.size(); j ++){
          if (files[i]->patch_file[j].is_valid) {
            live->insert(files[i]->patch_file[j].number);
          } 
          // else if (files[i]->patch_file[j].invalid_regions_count == files[i]->patch_file[j].total_regions_count) {
          //   files[i]->patch_file[j].is_valid = false;
          // }
        }
      }
    }
      ...
      if (c->level() + which == 0) {
        ...
        for (size_t i = 0; i < files.size(); i++) {
          list[num++] = table_cache_->NewMixedIterator(options, 
                  icmp_, &(c->input(which, i)->patch_file),
                  (c->input(which, i)->rim_->ValidRegion()));
        }
      } else {
        // Create concatenating iterator for the files from this level
        ///////////////////////////////////////
        list[num++] = NewTwoLevelIterator(
                new Version::LevelClusterNumIterator(icmp_, which, &c->inputs_[which]),
...
///////////////////////////////////////
// 2021_3_15
//Iterator* VersionSet::MyMakeInputIterator(bool isCluster, 
//                                          Iterator* iiter,
//                                          Iterator* jiter){
//
//  Iterator *result = new IteratorMerger(&icmp_, isCluster, iiter, jiter);
//  return result;
//}
///////////////////////////////////////
...
Compaction::Compaction(const Options* options, int level)
    : stage_(0), 
      ...
      ///////////////////////////////////////
      max_output_file_size_(OriginalClusterSizeForLevel(level)),
      ///////////////////////////////////////
      input_version_(NULL),
...
//   for (size_t i = 0; i < inputs_[1].size(); i++) {
//     if ( /*(level_ == 2 && inputs_[1][i]->patch_file.size() >= 10) 
//             || inputs_[1][i]->file_size >= ((level_+1)*MaxOutputFileSize())*/
//           inputs_[1][i]->patch_file.size() >= 16 
//             || inputs_[1][i]->file_size >= (10)*(inputs_[1][i]->patch_file[0].file_size) ) {
...
void Compaction::SetInputs1DropFlag() {
  for (size_t i = 0; i < inputs_[1].size(); i++) {
    if (inputs_[1][i]->patch_file.size() >= 16 
