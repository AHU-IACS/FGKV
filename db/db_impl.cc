...
std::vector<RegionInfoManager*> scan_b_;
...
struct DBImpl::CompactionState {
  ...
  struct Output {
    ...
    size_t which_file;
    // 是否是生成根表，在InstallCompactionResults中读取它的状态
    bool cluster;
    std::vector<PatchMetaData> patch_file;
    mutable RegionInfoManager* ri_m;
    std::vector<uint8_t> fmd_stage;

    Output() : cluster(false), ri_m(new RegionInfoManager()) {
    ...
    Output(const Output& outp) : number(outp.number), file_size(outp.file_size), 
            ...
            cluster(outp.cluster), ri_m(outp.ri_m) { 
      //ri_m->refs++;
      outp.ri_m = NULL;
      patch_file.assign(outp.patch_file.begin(), outp.patch_file.end());
      fmd_stage.assign(outp.fmd_stage.begin(), outp.fmd_stage.end());
      //ri_m = new RegionInfoManager(outp.ri_m);
    }
    ~Output() { 
...
Status DBImpl::WriteLevel0Table(MemTable* mem, VersionEdit* edit,
  ...
  meta.rim_ = new RegionInfoManager();
  meta.rim_->refs = 1;
  ...
  Status s;
  {
    ...
    PatchMetaData patch;
    patch.number = meta.number;
    patch.file_size = meta.file_size;
    patch.total_regions_count = meta.rim_->ValidRegion()->size();
    meta.patch_file.push_back(patch);
    ///////////////////////////////////////
    mutex_.Lock();
  }
  ...
  if (s.ok() && meta.file_size > 0) {
    ...
    edit->AddFile(level, meta.number, meta.file_size,
                  meta.smallest, meta.largest, meta.rim_, meta.patch_file, meta.fmd_stage_);
    ///////////////////////////////////////
    // // 用完要释放
    // delete meta.rim_;
  }
...
void DBImpl::PrintMyStats() {
  ...
  fprintf(stderr, "GetchosenRegions时间:\t %f \n", (fg_stats_->fg_getchosenregions_time * 1e-6));
  ...
  fprintf(stderr, "MinorCompaction次数:\t %lld \n", static_cast<unsigned long long>(
                fg_stats_->minor_compaction_count_in_l0 + 
                fg_stats_->minor_compaction_count_in_cluster + 
                fg_stats_->minor_compaction_count_in_patch));
  fprintf(stderr, "MinorCompaction时间:\t %f \n\n", (
                  (fg_stats_->minor_compaction_time_in_l0 + 
                  fg_stats_->minor_compaction_time_in_cluster + 
...
c->AddFMDStage(2, 2);
    ...
    c->edit()->AddFile(c->level() + 1, f->number, f->file_size, f->smallest, 
                        f->largest, f->rim_, f->patch_file, f->fmd_stage_);
    ///////////////////////////////////////
...
Status DBImpl::OpenCompactionOutputFile(CompactionState* compact,
  ...
  if (s.ok()) {
      ...
      ///////////////////////////////////////
      if(isCluster) {
          compact->builder = new TableBuilder(options_, compact->compaction->level()+1, compact->outfile, NULL, compact->current_output()->ri_m, NULL, fg_stats_);
      } else {
          compact->builder = new TableBuilder(options_, compact->compaction->level()+1, compact->outfile,
                    compact->compaction->input(1, order)->rim_, compact->current_output()->ri_m, 
                    &(compact->compaction->input(1, order)->patch_file), fg_stats_);
      }
      ///////////////////////////////////////
...
Status DBImpl::FinishCompactionOutputFile(CompactionState* compact,
                Iterator* iter, bool isCluster, bool isUsingCluster, size_t order) {
  ...
  if (s.ok()) {
    ...
    if (isCluster) {
      ...
      s = compact->builder->ClusterFinish(0);
      current_bytes = compact->builder->FileSize();
      PatchMetaData patch;
      patch.number = compact->current_output()->number;
      patch.file_size = current_bytes;
      patch.total_regions_count = compact->builder->flush_blocks_count;
      compact->current_output()->patch_file.push_back(patch);
    } else {
      ...
      if (isUsingCluster) {
        s = compact->builder->ClusterFinish(compact->compaction->input(1, order)->patch_file.size());
      } else {
        s = compact->builder->Finish(compact->compaction->input(1, order)->patch_file.size());
      }
      ...
      patch.file_size = current_bytes;
      patch.total_regions_count = compact->builder->flush_blocks_count;
      compact->compaction->input(1, order)->patch_file.push_back(patch);
  ...
  if (s.ok() && current_entries > 0) {
    ...
    //                                       &(compact->compaction->input(1, order)->patch_file), 
    //                                       compact->current_output()->ri_m->ValidRegion());
    // s = iter->status();
    // delete iter;
    // if (s.ok()) {
      if(isCluster){
          Log(options_.info_log,
          "Generated patch0 #%llu@%d: %lld keys, %lld bytes",
          ...
          Log(options_.info_log,
          "Generated patch[%lu] #%llu@%d: %lld keys, %lld bytes",
          compact->compaction->input(1, order)->patch_file.size() - 1,
          (unsigned long long) output_number,
          compact->compaction->level(),
          (unsigned long long) current_entries,
          (unsigned long long) current_bytes);
  ...
  for (size_t i = 0; i < compact->outputs.size(); i++) {
    ...
    if (out.cluster) {
      compact->compaction->edit()->AddFile(
        ...
        out.largest, out.ri_m, out.patch_file, out.fmd_stage);
    } else {
      size_t file = out.which_file;
      FileMetaData* f = compact->compaction->input(1, file);
      for (int j = 0; j < f->patch_file.size(); j++) {
        // if (f->patch_file[j].invalid_regions_count == f->patch_file[j].total_regions_count) {
        //   f->patch_file[j].is_valid = false;
        // }
        if (f->patch_file[j].invalid_size >= f->patch_file[j].file_size) {
          f->patch_file[j].is_valid = false;
        }
      }
      f->bkp_bim_ = out.ri_m;
...
Status DBImpl::DoCompactionWork(CompactionState* compact) {
  ...
  for (; input->Valid() && !shutting_down_.Acquire_Load(); ) {
    ...
    if (!drop) {
      ...
      ///////////////////////////////////////
      compact->builder->ClusterAdd(key, input->value(), 0);
      ///////////////////////////////////////
...
Status DBImpl::DoFineGrainedCompactionWork(CompactionState* compact) {
  ...
  std::vector< std::vector<RegionInfo*> > chosenRegion;
  ...
  versions_->GetchosenRegions(compact->compaction, &chosenRegion, iiter);
  iiter->SeekToFirst();
  int file = 0;
  Status status;

  for (; file < fileNum && !shutting_down_.Acquire_Load(); file++) {
    if (chosenRegion[file].empty()) continue;
    ...
    Iterator* jiter = table_cache_->NewMixedIterator(options, versions_->Icmp(),
              &(compact->compaction->input(1, file)->patch_file), 
              &chosenRegion[file]);
    ...
    bool isCluster = compact->compaction->fg_drop_[file];
    if (isCluster) {

      //// <COMPACTION_STAGE>
      compact->compaction->stage_ = 38;

      // fprintf(pout, "从表数: %d\t簇大小: %lld\n", compact->compaction->input(1, file)->patch_file.size(), 
      //     static_cast<unsigned long long>(compact->compaction->input(1, file)->file_size));
      // for (int cf = 0; cf < compact->compaction->input(1, file)->patch_file.size(); cf ++) {
      //   fprintf(pout, "%lld\t", 
      //     static_cast<unsigned long long>(compact->compaction->input(1, file)->patch_file[cf].file_size));
      // }
      // fprintf(pout, "\n");
      fg_stats_->total_patch_number_in_cluster_compaction += compact->compaction->input(1, file)->patch_file.size();

      //// <FMD_STAGE> <8, 8>
      // CLUSTER 进入单个FileMetaData遍历阶段
      compact->compaction->AddFMDStage(8, 8);

      //// <FG_STATS>
      uint64_t fg_cluster_compaction_start = env_->NowMicros();
      ...
      for (; input->Valid() && !shutting_down_.Acquire_Load(); ) {
        ...
        if (has_imm_.NoBarrier_Load() != NULL) {
          ...
          if (imm_ != NULL) {
            ...
            //// <FG_STATS>
            fg_stats_->minor_compaction_count_in_cluster += 1;
          ...
          //// <FG_STATS>
          fg_stats_->minor_compaction_time_in_cluster += (env_->NowMicros() - imm_start);
        ...
        if (compact->compaction->ShouldStopBefore(key) &&
            compact->builder != NULL) {
          status = FinishCompactionOutputFile(compact, input, isCluster, false, file);
          if (!status.ok()) {
            break;
          }
        }
        ...
        if (!drop) {
          // Open output file if necessary
          if (compact->builder == NULL) {
            status = OpenCompactionOutputFile(compact, isCluster, file);
            if (!status.ok()) {
              break;
            }
          }
          ...
          //// <COMPACTION_STAGE>
          // 40标识cluster compaction中的builder->ClusterAdd
        ...
        //// <COMPACTION_STAGE>
        // 40标识cluster compaction中的builder->Add
      ...
      if ( (file == fileNum-1) && iiter->Valid() && !shutting_down_.Acquire_Load() ) { 
        ...
        while (iiter->Valid()) {
          key = iiter->key();
          compact->current_output()->largest.DecodeFrom(key);
          compact->builder->ClusterAdd(key, iiter->value(), 0);
          iiter->Next();
        }
        if (status.ok() && shutting_down_.Acquire_Load()) {
          status = Status::IOError("Deleting DB during compaction");
      ...
      //// <FG_STATS>
      fg_stats_->fg_cluster_compaction_count += 1;
      fg_stats_->fg_cluster_compaction_time += 
              (env_->NowMicros() - fg_cluster_compaction_start);

    } else {

      //// <FMD_STAGE> <10, 10>
      // PATCH 进入单个FileMetaData遍历阶段
      compact->compaction->AddFMDStage(10, 10);

      //// <FG_STATS>
      uint64_t fg_patch_compaction_start = env_->NowMicros();

      int patch = compact->compaction->input(1, file)->patch_file.size();
      for (; input->Valid() && !shutting_down_.Acquire_Load(); ) {
        ...
        if (has_imm_.NoBarrier_Load() != NULL) {
          ...
          //// <COMPACTION_STAGE>
          // 61标识patch compaction中的CompactMemTable
          ...
          if (imm_ != NULL) {
            ...
            //// <FG_STATS>
            fg_stats_->minor_compaction_count_in_patch += 1;
          ...
          //// <FG_STATS>
          fg_stats_->minor_compaction_time_in_patch += (env_->NowMicros() - imm_start);
        ...
        if (!drop) {
          ...
          //// <COMPACTION_STAGE>
          // 46标识patch compaction中的DecodeFrom
          ...
          //// <COMPACTION_STAGE>
          // 47标识patch compaction中的builder->Add
          compact->compaction->stage_ = 47;

          compact->builder->Add(key, input->value(), patch);
        }

        //// <COMPACTION_STAGE>
        // 48标识patch compaction中的builder->Add
      ...
      if ( (file == fileNum-1) && iiter->Valid() && !shutting_down_.Acquire_Load() ) {
        ...
        compact->builder->UpdateIndexEntry(patch);
        while (iiter->Valid()) {
          Slice key = iiter->key();
          compact->current_output()->largest.DecodeFrom(key);
          compact->builder->ClusterAdd(key, iiter->value(), patch);
          iiter->Next();
        }
        if (status.ok() && shutting_down_.Acquire_Load()) {
          status = Status::IOError("Deleting DB during compaction");
        ...
        if (status.ok() && compact->builder != NULL) {
          status = FinishCompactionOutputFile(compact, input, isCluster, true, file);
        }
      ...
      //// <FG_STATS>
      fg_stats_->fg_patch_compaction_count += 1;
      fg_stats_->fg_patch_compaction_time += 
              (env_->NowMicros() - fg_patch_compaction_start);
