...
///////////////////////////////////////
Status TableCache::FG_FindIndexKey(FileMetaData* f, const Slice& k, RegionInfo* b) { 
  ...
  std::vector<RegionInfo*>* validBlock = &(f->rim_->fg_valid_region_);
  uint64_t left = 0;
...
Status TableCache::Get(const ReadOptions& options,
  ...
  if (s.ok()) { 
    uint64_t fileNumber = f->patch_file[bInfo.file].number;
    uint64_t fileSize = f->patch_file[bInfo.file].file_size;
...
Iterator* TableCache::NewMixedIterator(
                      ...
                      const InternalKeyComparator& icmp,
									    std::vector<PatchMetaData>* patchFile, 
									    std::vector<RegionInfo*>* chosenRegion) {

  std::vector<Cache::Handle*> handle;
  //std::vector<Cache::Handle**> handle; ?
  std::vector<Table*> table;
  for(int i = 0; i < patchFile->size(); i++){
    if ((*patchFile)[i].is_valid) {
      Cache::Handle* h = NULL;
      Status s = FindTable((*patchFile)[i].number, (*patchFile)[i].file_size, &h);
      if (!s.ok()) {
        return NewErrorIterator(s);
      }
  ...
  Iterator* result = NewClusterIterator(options, icmp, table, chosenRegion);
